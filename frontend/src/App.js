...
function App() {
  ...
  const handleAuth = async () => {
    ...
    try {
        ...
        setUser(res.data);
    } catch (err) { alert("AUTH_FAILED: Check backend/database."); }
  };
  ...
  if (view === 'landing' && !user) {
    return (
      <div className={`min-h-screen ${bgMain} ${txtMain} flex flex-col items-center justify-center font-mono p-6 transition-all duration-700`}>
        ...
        <button onClick={() => setView('auth')} className={`px-20 py-8 font-black uppercase tracking-[0.5em] skew-x-[-15deg] transition-all hover:scale-110 shadow-2xl ${btnPrimary}`}>Initiate_Link</button>
      </div>
    );
  }
    ...
    <div className={`min-h-screen ${bgMain} ${txtMain} font-mono flex transition-all duration-500`}>
      <nav className={`w-64 border-r flex flex-col items-center py-8 gap-4 ${isLight ? 'bg-white border-slate-200' : 'bg-black/60 border-white/5'}`}>
        ...
        <div className="mt-auto w-full px-6 flex flex-col gap-4">
            ...
            <button onClick={logout} className={`w-full py-4 text-[10px] font-black uppercase border transition-all ${isLight ? 'border-red-200 text-red-500' : 'border-neon-pink text-neon-pink hover:bg-neon-pink hover:text-black'}`}><LogOut size={12} className="inline mr-2"/> Logout</button>
        </div>
      </nav>
         ...
         {activeTab === 'overview' && (
            <div className="animate-in fade-in duration-700">
                <div className="grid grid-cols-3 gap-8 mb-12">
                    <div className={`p-8 border-2 ${cardBase}`}><h2 className="text-[10px] font-black mb-4 uppercase opacity-40">Total_Hours</h2><p className="text-7xl font-black italic">{data.total_hours.toFixed(1)}</p></div>
                    <div className={`p-8 border-2 ${cardBase}`}><h2 className={`text-[10px] font-black mb-4 uppercase ${isLight ? 'opacity-40' : 'text-neon-blue'}`}>Streak</h2><p className={`text-7xl font-black italic ${isNeon ? 'text-neon-blue' : ''}`}>{data.streak}D</p></div>
                    <div className={`p-8 border-2 ${cardBase}`}><h2 className={`text-[10px] font-black mb-4 uppercase ${isNeon ? 'text-neon-pink' : 'opacity-40'}`}>Efficiency</h2><p className={`text-7xl font-black italic ${isNeon ? 'text-neon-pink' : ''}`}>{data.efficiency}%</p></div>
                </div>
                <div className={`p-10 border-2 ${cardBase}`}>
                    <h3 className="text-xs font-black mb-10 uppercase tracking-widest opacity-50 italic">Upload_Log</h3>
                    <div className="grid grid-cols-5 gap-8">
                        <input value={subject} onChange={(e) => setSubject(e.target.value)} type="text" placeholder="SUBJECT" className={`col-span-2 border p-5 font-bold uppercase ${isLight ? 'bg-slate-50' : 'bg-black/40 text-white'}`} />
                        <input value={hours} onChange={(e) => setHours(e.target.value)} type="number" placeholder="HRS" className={`col-span-1 border p-5 font-bold ${isLight ? 'bg-slate-50' : 'bg-black/40 text-white'}`} />
                        <button onClick={handleUpload} className={`col-span-2 font-black py-5 uppercase text-sm ${btnPrimary}`}>Transmit</button>
                    </div>
                </div>
            </div>
         )}
         {activeTab === 'analytics' && (
            <div className={`p-12 border-2 h-[550px] ${cardBase}`}><ResponsiveContainer><PieChart><Pie data={data.chartData} innerRadius={100} outerRadius={160} paddingAngle={8} dataKey="value" stroke="none">{data.chartData.map((e, i) => <Cell key={i} fill={COLORS[i % COLORS.length]} />)}</Pie><Tooltip contentStyle={{backgroundColor: isLight ? '#fff' : '#000'}} /><Legend /></PieChart></ResponsiveContainer></div>
         )}
         {activeTab === 'focus' && (
            <div className="flex flex-col items-center justify-center h-[500px]">
                <div className={`text-[12rem] font-black italic tracking-tighter mb-12 ${isNeon ? 'text-white drop-shadow-[0_0_30px_rgba(255,255,255,0.3)]' : ''}`}>{Math.floor(timeLeft/60)}:{String(timeLeft%60).padStart(2,'0')}</div>
                <button onClick={() => setIsActive(!isActive)} className={`px-20 py-8 font-black uppercase skew-x-[-12deg] ${btnPrimary}`}>{isActive ? 'Pause' : 'Start'}</button>
            </div>
         )}
         {activeTab === 'history' && (
            <div className={`p-10 border-2 ${cardBase}`}>
                <table className="w-full text-left">
                    <thead className="text-[10px] uppercase opacity-40 border-b border-black/10"><tr><th className="py-6">TIME</th><th className="py-6">MODULE</th><th className="py-6">DURATION</th></tr></thead>
                    <tbody className="text-xs font-black">{data.history.map(log => (<tr key={log.id} className="border-b border-black/5"><td className="py-8 opacity-40">{log.date}</td><td className="py-8 uppercase italic">{log.subject}</td><td className={`py-8 text-xl font-black ${isNeon ? 'text-neon-pink' : ''}`}>{log.hours} HRS</td></tr>))}</tbody>
                </table>
            </div>
         )}
      </main>
